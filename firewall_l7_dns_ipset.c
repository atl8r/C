 ...
 *
 * Kompiliere mit:
 *   gcc -O2 -Wall -Wextra -pthread -o firewall_l7_dns_ipset firewall_l7_dns_ipset.c \
 *   -lpcap -lnghttp2 -lmnl
 *
...
static inline int hash_ip(af_t af, const ip_addr *ip) {
    return (af == AF_4) ? siphash24((uint8_t*)&ip->v4, 4) % MAX_ENTRIES :
...
// --- IP-Set Funktionen ---
int ipset_add(af_t af, const ip_addr *ip, time_t timeout) {
    ...
    mnl_attr_put_strz(nlh, IPSET_ATTR_SETNAME, IPSET_NAME);
    mnl_attr_put_u32(nlh, IPSET_ATTR_TIMEOUT, timeout);
...
int ipset_del(af_t af, const ip_addr *ip) {
    char buf[MNL_SOCKET_BUFFER_SIZE];
    struct nlmsghdr *nlh = mnl_nlmsg_put_header(buf);
...
void ipset_init() {
    ...
    if (!nl || mnl_socket_bind(nl, 0, MNL_SOCKET_AUTOPID) < 0) {
        syslog(LOG_ERR, "IP-Set init failed");
...
// --- DNS Query Parsing ---
int parse_dns_query(const uint8_t *payload, int len, char *domain_out, size_t domain_max) {
    if (len < 12) return 0;
    size_t pos = 12, out_pos = 0;
    while (pos < len && payload[pos] != 0) {
        uint8_t label_len = payload[pos++];
        if (pos + label_len > len) return 0;
        if (out_pos + label_len + 1 >= domain_max) return 0;
        if (out_pos > 0) domain_out[out_pos++] = '.';
        memcpy(domain_out + out_pos, payload + pos, label_len);
        out_pos += label_len;
        pos += label_len;
    }
...
void check_dns_tunnel(af_t af, const ip_addr *src_ip, const uint8_t *payload, int len) {
    char domain[256];
    if (!parse_dns_query(payload, len, domain, sizeof(domain))) return;

    int subdomain_len = strlen(domain);
    if (strchr(domain, '.')) {
        char *sub = strrchr(domain, '.');
        if (sub > domain) subdomain_len = sub - domain;
    }

    double entropy = shannon_entropy((uint8_t*)domain, subdomain_len);
    char ip_str[INET6_ADDRSTRLEN];
    inet_ntop(af == AF_4 ? AF_INET : AF_INET6, af == AF_4 ? (void*)&src_ip->v4 : (void*)&src_ip->v6, ip_str, sizeof(ip_str));

    if (entropy > DNS_ENTROPY_THRESHOLD && subdomain_len > DNS_SUBDOMAIN_MAX) {
        live_log("1;35", "DNSTUN", "High entropy + long subdomain", ip_str, domain);
    ...
    if (!r) {
        ...
        if (r) { r->af = af; r->ip = *src_ip; r->last_dns_ts = now; r->next = *head; *head = r; }
    }
    if (r) {
        ...
        if (r->dns_queries > DNS_RATE_THRESHOLD) {
            live_log("1;35", "DNSFLOOD", "High query rate", ip_str, NULL);
...
static int on_header_callback(nghttp2_nv *nv, void *user_data) {
    if (nv->namelen > 1024 || nv->valuelen > 4096) return NGHTTP2_ERR_CALLBACK_FAILURE;
    return 0;
}
...
void analyze_http2(const uint8_t *data, size_t len, af_t af, ip_addr *src_ip, tcp_session *sess) {
    ...
    while (pos + HTTP2_FRAME_HEADER_LEN <= len) {
        ...
        if (type == HTTP2_HEADERS_FRAME && plen > 0) {
            ...
            while (inlen > 0) {
                ...
                if (rv < 0) { update_rate_and_block(af, src_ip, 0, "HPACK Error"); return; }
                in += rv; inlen -= rv;
    ...
    if (headers > 20) update_rate_and_block(af, src_ip, 0, "H2 Header Flood");
}

void analyze_quic(const u_char *payload, int len, af_t af, ip_addr *src_ip) {
    if (len <= 0 || (payload[0] & 0x80) != QUIC_LONG_HEADER || len < 50) return;
    if (parse_tls_alpn(payload, len, ALPN_H3)) {
        char ip_str[INET6_ADDRSTRLEN];
        inet_ntop(af == AF_4 ? AF_INET : AF_INET6, af == AF_4 ? (void*)&src_ip->v4 : (void*)&src_ip->v6, ip_str, sizeof(ip_str));
        live_log("1;35", "H3", "HTTP/3 (QUIC)", ip_str, NULL);
        syslog(LOG_INFO, "HTTP/3 from %s", ip_str);
    }
}
...
void process_tcp_payload(tcp_session *sess, const u_char *payload, int payload_len, af_t af, ip_addr *src_ip) {
    ...
    if (sess->data_len + payload_len > MAX_SESSION_DATA) { update_rate_and_block(af, src_ip, 0, "Payload Flood"); return; }
    ...
    if (sess->data_len + payload_len > sess->alloc_len) {
        ...
        if (!new) return;
        sess->data = new; sess->alloc_len = new_len;
    }
    ...
    if (!sess->is_http2 && sess->data_len >= HTTP2_MAGIC_LEN && is_http2_magic(sess->data)) {
        sess->is_http2 = 1;
        live_log("1;33", "H2C", "HTTP/2 clear", ip_str, NULL);
        syslog(LOG_INFO, "HTTP/2 (h2c) from %s", ip_str);
    }
    if (!sess->is_http2 && sess->data_len >= 60 && sess->data[0] == TLS_HANDSHAKE && parse_tls_alpn((uint8_t*)sess->data, sess->data_len, ALPN_H2)) {
        sess->is_http2 = 1;
        live_log("1;33", "H2", "HTTP/2 TLS", ip_str, NULL);
        syslog(LOG_INFO, "HTTP/2 (h2) from %s", ip_str);
    }
...
void update_rate_and_block(af_t af, const ip_addr *ip, int is_dns, const char *reason) {
    ...
    ip_rate *r = *head;
    int bucket_size = 0;
    while (r && (r->af != af || memcmp(&r->ip, ip, af == AF_4 ? 4 : 16))) { r = r->next; bucket_size++; }
    if (bucket_size > MAX_HASH_BUCKET) { pthread_mutex_unlock(&lock); return; }
    ...
    if (r->count > RATE_THRESHOLD || r->offenses > 5) {
        if (block_count >= MAX_BLOCK_ENTRIES) { pthread_mutex_unlock(&lock); return; }

        if (nl && ipset_add(af, ip, BLOCK_DURATION) == 0) {
            ...
            if (b) {
                ...
                live_log("1;31", "BLOCK", ip_str, reason, "via ipset");
                syslog(LOG_WARNING, "BLOCKED %s: %s (ipset)", ip_str, reason);
            }
        }
    }
    r->offenses++;
...
void packet_handler(u_char *user, const struct pcap_pkthdr *h, const u_char *bytes) {
    ...
    if (proto == IPPROTO_UDP && payload_len >= 8) {
        const struct udphdr *udp = (struct udphdr*)payload;
        uint16_t dport = ntohs(udp->dest);
        if (dport == 53 && payload_len > 8) {
            check_dns_tunnel(af, &src_ip, payload + 8, payload_len - 8);
        } else if (dport == 443) {
            int quic_len = payload_len - 8;
            if (quic_len > 0) analyze_quic(payload + 8, quic_len, af, &src_ip);
        }
    } else if (proto == IPPROTO_TCP && payload_len >= 20) {
        ...
        pthread_mutex_lock(&lock);
        if (session_count >= MAX_TCP_SESSIONS) { pthread_mutex_unlock(&lock); return; }
        ...
        if (!sess) {
            ...
            if (!sess || !(sess->data = malloc(sess->alloc_len = 32768))) {
                free(sess); pthread_mutex_unlock(&lock); return;
            }
            sess->af = af; sess->src_ip = src_ip; sess->src_port = sport; sess->dst_port = dport;
            sess->next = tcp_sessions; tcp_sessions = sess; session_count++;
        }
    ...
    while (running) {
        ...
        while (*p) {
            if (now - (*p)->last_seen > SESSION_TIMEOUT) {
                ...
                free(s->data); free(s); session_count--;
            } else p = &(*p)->next;
        }
...
int main(int argc, char **argv) {
    ...
    if (pcap_compile(handle, &fp, "tcp or udp port 443 or udp port 53", 0, PCAP_NETMASK_UNKNOWN) == -1 ||
    ...
    live_log("1;36", "START", "L7 Firewall + DNS-Tunnel + IP-Set", iface, "DNS + H2/H3");
    syslog(LOG_INFO, "Firewall gestartet auf %s", iface);
